...
# 启动全部服务（frontend / backend / mysql / redis）
docker-compose up -d
```

访问 http://localhost

启动时后端会自动执行迁移：补齐 `users.role` 列、修正演示账号密码、创建演示额度、在空表时插入 6 条网站媒体 + 5 条自媒体示例数据。演示账号默认被设置为 `admin` 角色。
...
### 个人中心
- 消耗明细（点数 + 余额）、实名认证、账号权益总览、账户充值、修改密码
